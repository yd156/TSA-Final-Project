...
dailyprice.train.ts <- subset(dailyprice.ts,
                                   end = length(dailyprice.ts)-365) #Jan 1st 2018 to Dec 30st 2021
...
ETS_plot<-autoplot(dailyprice.ts, color = "dark grey") +
...
ARIMA_origin<- auto.arima(dailyprice.train.ts)
ARIMA_load <- auto.arima(dailyprice.train.ts,xreg=fourier(dailyload.train.ts,K=c(2,18)))
ARIMA_ng<- auto.arima(dailyprice.train.ts,xreg=fourier(NGprice.fixed.train.ts,K=c(2,18)))
ARIMA_ng&load <- auto.arima(dailyprice.train.ts,xreg=fourier(dailyload.train.ts,NGprice.fixed.train.ts,K=c(2,18)))

dailyload.train.ts_fc <- forecast(dailyload.train.ts,NGprice.fixed.train.ts, h = 365,level = 0.90)
NGprice.fixed.train.ts_fc <- forecast(dailyload.train.ts, h = 365)

ARIMA_origin_fc <- forecast(object = ARIMA_origin,h=365)
ARIMA_load_fc <- forecast(object = ARIMA_origin_fc,xreg=fourier(dailyload.train.ts_fc$mean,K=c(2,6),h=365),h=365)
ARIMA_ng_fc <- forecast(object = ARIMA_origin_fc,xreg= fourier(NGprice.fixed.train.ts_fc$mean,K=c(2,6),h=365),h=365)

#Plot forecast with observed data
ARIMA_plot <- autoplot(dailyprice.ts, color = "dark grey") +
  autolayer(ARIMA_fc$mean, series="ARIMA_xgre",PI=FALSE,color = "blue") +
  ylab("Electricity Price Forecast")
ARIMA_plot

ARIMA_load_plot <- autoplot(dailyprice.ts, color = "dark grey") +
  autolayer(ARIMA_load_fc$mean, series="ARIMA_xgre",PI=FALSE,color = "blue") +
  ylab("Electricity Price Forecast with EXO(load)")
ARIMA_load_plot 

ARIMA_ng_plot <-autoplot(dailyprice.ts, color = "dark grey") +
...
## Neural Network
```{r, Neural Network}
NN_origin <- nnetar(dailyprice.train.ts,p=1,P=0)
NN_load <- nnetar(dailyprice.train.ts,p=1,P=0,xreg=fourier(dailyload.train.ts,K=c(2,18)))
NN_ng <- nnetar(dailyprice.train.ts,p=1,P=0,xreg=fourier(NGprice.fixed.train.ts,K=c(2,18)))


#Forecast with NNet
NN_origin_fc<- forecast(NN_origin, h=365,K=c(2,18))
NN_load_fc <- forecast(NN_load, h=365,xreg=fourier(dailyload.train.ts_fc$mean,K=c(2,18)))
NN_ng_fc <-forecast(NN_ng, h=365,xreg=fourier(NGprice.fixed.train.ts,K=c(2,18)))
...
autoplot(dailyprice.test.ts) +
  ...
  autolayer(ARIMA_origin_fc, PI=FALSE, series="ARIMA + Fourier") +
  autolayer(ARIMA_load_fc, PI=FALSE, series="ARIMA + EXO")
  autolayer(TBATS_fc,PI=FALSE, series="TBATS") +
  autolayer(NN_origin_fc,PI=FALSE, series="NN") +
  autolayer(NN_load_fc,PI=FALSE, series="NN+EXO") +
  autolayer(ARIMA_forecast, PI=FALSE, series="ARIMA + Fourier") +
  autolayer(TBATS_forecast,PI=FALSE, series="TBATS") +
  autolayer(NN_forecast,PI=FALSE, series="NN") +
